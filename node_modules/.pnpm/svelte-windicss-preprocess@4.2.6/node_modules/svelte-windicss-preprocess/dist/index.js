...
var init_utils = __esm({
  "node_modules/mlly/lib/_utils.mjs"() {
    init_cjs_shims();
    import_node_module = __toModule(require("module"));
...
function fileURLToPath(id2) {
  if (typeof id2 === "string" && !id2.startsWith("file://")) {
    return normalizeSlash(id2);
  }
  return normalizeSlash((0, import_node_url.fileURLToPath)(id2));
}
...
function createCommonJS(url) {
  ...
  function require3(id2) {
    return getNativeRequire()(id2);
  }
  require3.resolve = (id2, options) => getNativeRequire().resolve(id2, options);
  return {
...
var init_cjs = __esm({
  "node_modules/mlly/lib/cjs.mjs"() {
    init_cjs_shims();
    import_node_module2 = __toModule(require("module"));
...
var init_resolve = __esm({
  "node_modules/mlly/lib/resolve.mjs"() {
    init_cjs_shims();
    import_node_fs2 = __toModule(require("fs"));
...
var import_path, import_fs, require2;
...
async function loadCollectionFromFS(name) {
  ...
  async function task() {
    ...
    if (jsonPath) {
      const icons = JSON.parse(await import_fs2.promises.readFile(jsonPath, "utf8"));
      return icons;
    } else {
      return void 0;
    }
...
var import_fs2, _collections, isLegacyExists;
var init_fs = __esm({
  "node_modules/@unocss/preset-icons/dist/fs.mjs"() {
    init_cjs_shims();
...
function entriesToCss(arr) {
  if (arr == null)
    return "";
  return arr.map(([key, value2]) => value2 != null ? `${key}:${value2};` : void 0).filter(Boolean).join("");
}
...
function resolveConfig(userConfig = {}, defaults3 = {}) {
  const config = Object.assign({}, defaults3, userConfig);
...
var UnoGenerator = class {
  constructor(userConfig = {}, defaults3 = {}) {
    ...
    this.config = resolveConfig(userConfig, defaults3);
  }
  setConfig(userConfig, defaults3) {
    if (!userConfig)
      return;
    if (defaults3)
      this.defaults = defaults3;
  ...
  async applyExtractors(code, id2, set = new Set()) {
    const context = {
      get original() {
        return code;
      },
  ...
  async generate(input, {
    ...
    const tokens = typeof input === "string" ? await this.applyExtractors(input, id2) : input;
    if (safelist)
    ...
    const sheet = new Map();
    const hit = (raw2, payload) => {
      var _a;
      this._cache.set(raw2, payload);
      matched.add(raw2);
    ...
    const block = (raw2) => {
      this.blocked.add(raw2);
      this._cache.set(raw2, null);
    };
    await Promise.all(Array.from(tokens).map(async (raw2) => {
      if (matched.has(raw2) || this.blocked.has(raw2))
        return;
      if (this._cache.has(raw2)) {
        const r = this._cache.get(raw2);
        if (r)
          hit(raw2, r);
        return;
      }
      ...
      if (this.config.preprocess)
        current2 = this.config.preprocess(raw2);
      if (this.isBlocked(current2))
        return block(current2);
      const applied = this.matchVariants(raw2, current2);
      ...
      if (expanded) {
        const utils2 = await this.stringifyShortcuts(applied, context, expanded[0], expanded[1]);
        if (utils2 == null ? void 0 : utils2.length)
          return hit(raw2, utils2);
      } else {
        const util = this.stringifyUtil(await this.parseUtil(applied, context));
        if (util)
          return hit(raw2, [util]);
      }
      this._cache.set(raw2, null);
    }));
    if (preflights) {
    ...
    const getLayer = (layer) => {
      ...
      let css = Array.from(sheet).sort((a, b) => (this.parentOrders.get(a[0]) || 0) - (this.parentOrders.get(b[0]) || 0)).map(([parent, items]) => {
        ...
        }).map((a) => [a[1] ? applyScope(a[1], scope2) : a[1], a[2]]);
        if (!sorted.length)
          return void 0;
        const rules = sorted.map(([selector2, body], idx) => {
          if (selector2 && this.config.mergeSelectors) {
            for (let i = size - 1; i > idx; i--) {
              const current2 = sorted[i];
              if (current2 && current2[0] && current2[1] === body) {
                current2[0] = `${selector2},${current2[0]}`;
                return null;
              }
            }
          }
          return selector2 ? `${selector2}{${body}}` : body;
  ...
  matchVariants(raw2, current2) {
    const usedVariants = new Set();
    const handlers2 = [];
    let processed = current2 || raw2;
    let applied = false;
    while (true) {
      applied = false;
      for (const v of this.config.variants) {
        if (!v.multiPass && usedVariants.has(v))
          continue;
        let handler = v.match(processed, raw2, this.config.theme);
        if (!handler)
          continue;
      ...
      if (!applied)
        break;
      if (handlers2.length > 500)
        throw new Error(`Too many variants applied to "${raw2}"`);
    }
    return [raw2, processed, handlers2];
  }
  applyVariants(parsed, variantHandlers = parsed[4], raw2 = parsed[1]) {
    return [
      variantHandlers.reduce((p, v) => {
        var _a;
        return ((_a = v.selector) == null ? void 0 : _a.call(v, p)) || p;
      }, toEscapedSelector(raw2)),
      variantHandlers.reduce((p, v) => {
        var _a;
  ...
  constructCustomCSS(context, body, overrideSelector) {
    ...
    const [selector2, entries, mediaQuery] = this.applyVariants([0, overrideSelector || context.rawSelector, body, void 0, context.variantHandlers]);
  ...
  async parseUtil(input, context, internal = false) {
    var _a, _b;
    const [raw2, processed, variantHandlers] = typeof input === "string" ? this.matchVariants(input) : input;
    const staticMatch = this.config.rulesStaticMap[processed];
    if (staticMatch) {
      if (staticMatch[1] && (internal || !((_a = staticMatch[2]) == null ? void 0 : _a.internal)))
        return [staticMatch[0], raw2, normalizeEntries(staticMatch[1]), staticMatch[2], variantHandlers];
    }
    ...
    for (let i = rulesSize; i >= 0; i--) {
      ...
      if (entries.length)
        return [i, raw2, entries, meta, variantHandlers];
    }
  }
  stringifyUtil(parsed) {
    ...
    const [selector2, entries, mediaQuery] = this.applyVariants(parsed);
    const body = entriesToCss(entries);
    if (!body)
      return;
    return [parsed[0], selector2, body, mediaQuery, parsed[3]];
  }
  ...
  async stringifyShortcuts(parent, context, expanded, meta = { layer: this.config.shortcutsLayer }) {
    ...
    const [raw2, , parentVariants] = parent;
    for (const item of parsed) {
      if (isRawUtil(item))
        continue;
      const [selector2, entries, mediaQuery] = this.applyVariants(item, [...item[4], ...parentVariants], raw2);
      const mapItem = selectorMap.getFallback(selector2, mediaQuery, [[], item[0]]);
      mapItem[0].push(...entries);
      if (item[0] > mapItem[1])
        mapItem[1] = item[0];
    }
    return selectorMap.map(([entries, index], selector2, mediaQuery) => {
      const body = entriesToCss(entries);
      if (body)
        return [index, selector2, body, mediaQuery, meta];
      return void 0;
    }).filter(Boolean);
  }
  isBlocked(raw2) {
    return !raw2 || this.config.blocklist.some((e2) => typeof e2 === "string" ? e2 === raw2 : e2.test(raw2));
  }
};
function createGenerator(config, defaults3) {
  return new UnoGenerator(config, defaults3);
}
var reScopePlaceholder = / \$\$ /;
var hasScopePlaceholder = (css) => css.match(reScopePlaceholder);
function applyScope(css, scope2) {
  if (hasScopePlaceholder(css))
    return css.replace(reScopePlaceholder, scope2 ? ` ${scope2} ` : " ");
  else
    return scope2 ? `${scope2} ${css}` : css;
}
function toEscapedSelector(raw2) {
  if (raw2.startsWith("["))
    return raw2.replace(/^\[(.+?)(~?=)"(.*)"\]$/, (_, n2, s, i) => `[${e(n2)}${s}"${e(i)}"]`);
  else
    return `.${e(raw2)}`;
}
...
// node_modules/@iconify/utils/lib/icon-set/get-icon.mjs
function getIconData(data2, name, full = false) {
  function getIcon(name2, iteration) {
    ...
    if (((_a = data2.aliases) == null ? void 0 : _a[name2]) !== void 0) {
      const item = (_b = data2.aliases) == null ? void 0 : _b[name2];
    ...
    if (iteration === 0 && ((_c = data2.chars) == null ? void 0 : _c[name2]) !== void 0) {
      return getIcon((_d = data2.chars) == null ? void 0 : _d[name2], iteration + 1);
    }
    return null;
  }
...
async function searchForIcon(collection, id2, collections, scale) {
  let iconSet = collections[collection];
  if (typeof iconSet === "function")
    iconSet = await iconSet();
  if (!iconSet && isNode) {
    const { loadCollectionFromFS: loadCollectionFromFS2 } = await Promise.resolve().then(() => (init_fs(), fs_exports));
    iconSet = await loadCollectionFromFS2(collection);
  }
  if (!iconSet)
    return;
  const iconData = getIconData(iconSet, id2, true);
  if (iconData) {
...
var get_comment_handlers = (comments, raw2) => ({
  onComment: (block, value2, start, end) => {
    if (block && /\n/.test(value2)) {
      ...
      while (/[ \t]/.test(raw2[b]))
        b += 1;
      const indentation = raw2.slice(a, b);
...
${state.indent}	` : ", ");
  if (multiple_lines) {
    chunks.push(...join2(declarators, separator));
  } else {
    chunks.push(...join2(declarators, separator));
  }
  return chunks;
...
${state.indent}}`)
  ...
  SwitchStatement(node2, state) {
    ...
    node2.cases.forEach((block) => {
      if (block.test) {
        chunks.push(c(`
${state.indent}	case `), ...handle(block.test, __spreadProps(__spreadValues({}, state), { indent: `${state.indent}	` })), c(":"));
      } else {
        chunks.push(c(`
${state.indent}	default:`));
      ...
      block.consequent.forEach((statement) => {
        chunks.push(c(`
${state.indent}		`), ...handle(statement, __spreadProps(__spreadValues({}, state), { indent: `${state.indent}		` })));
      });
    });
    chunks.push(c(`
${state.indent}}`));
  ...
  FunctionDeclaration: scoped((node2, state) => {
    ...
    chunks.push(c("("));
    const params = node2.params.map((p) => handle(p, __spreadProps(__spreadValues({}, state), {
      indent: state.indent + "	"
    })));
...
${state.indent}` : ", ");
    if (multiple_lines) {
      chunks.push(c(`
${state.indent}	`), ...join2(params, separator), c(`
${state.indent}`));
    } else {
      chunks.push(...join2(params, separator));
    }
    chunks.push(c(") "), ...handle(node2.body, state));
    return chunks;
  }),
  ...
  ImportDeclaration(node2, state) {
	...
	`), ...join2(specifiers, c(",\n	")), c("\n}"));
          ...
          chunks.push(c(`{ `), ...join2(specifiers, c(", ")), c(" }"));
        }
      }
      chunks.push(c(" from "));
    }
  ...
  ArrowFunctionExpression: scoped((node2, state) => {
    ...
    if (node2.params.length === 1 && node2.params[0].type === "Identifier") {
      ...
      chunks.push(c("("), ...join2(params, c(", ")), c(")"));
    }
    chunks.push(c(" => "));
  ...
  ArrayExpression(node2, state) {
    ...
    if (multiple_lines) {
      chunks.push(c(`
${state.indent}	`), ...join2(elements, c(`,
${state.indent}	`)), c(`
${state.indent}`), ...sparse_commas);
    } else {
      chunks.push(...join2(elements, c(", ")), ...sparse_commas);
    }
    chunks.push(c("]"));
    return chunks;
  },
  ObjectExpression(node2, state) {
    ...
    node2.properties.forEach((p, i) => {
      chunks.push(...handle(p, __spreadProps(__spreadValues({}, state), {
        indent: state.indent + "	"
      })));
...
${state.indent}}` : ` }`)
  ...
  Property(node2, state) {
    ...
    if (node2.value.type === "FunctionExpression" && !node2.value.id) {
      ...
      chunks.push(...node2.computed ? [c("["), ...key, c("]")] : key, c("("), ...join2(node2.value.params.map((param) => handle(param, state)), c(", ")), c(") "), ...handle(node2.value.body, state));
      return chunks;
    }
    if (node2.computed) {
      return [
  ...
  SequenceExpression(node2, state) {
    const expressions = node2.expressions.map((e2) => handle(e2, state));
    return [
      c("("),
      ...join2(expressions, c(", ")),
      c(")")
...
${state.indent}: `), ...alternate);
  ...
  NewExpression(node2, state) {
    ...
    chunks.push(c("("), ...join2(args, separator), c(")"));
    return chunks;
  },
  ...
  CallExpression(node2, state) {
    ...
    if (multiple_lines) {
      ...
      chunks.push(c(`(
${state.indent}	`), ...join2(args2, c(`,
${state.indent}	`)), c(`
${state.indent})`));
    } else {
      chunks.push(c("("), ...join2(args, c(", ")), c(")"));
    }
    return chunks;
  },
  ...
  Literal(node2, state) {
    if (typeof node2.value === "string") {
      return [
        c((node2.raw || JSON.stringify(node2.value)).replace(re, (_m, _i, at2, hash, name) => {
          if (at2)
            return "@" + name;
          if (hash)
            return "#" + name;
...
var inject = (raw2, node2, values, comments) => {
...
function uniqInStr(str) {
  let uniq2 = String(Math.random()).slice(2);
  while (str.indexOf(uniq2) > -1) {
    uniq2 += uniq2;
  }
  return uniq2;
}
...
function globalStyleSheet(styleSheet) {
  styleSheet.children.forEach((style) => {
    var _a;
    if (!style.rule.includes(":global") && style.meta.group !== "keyframes" && !((_a = style.atRules) == null ? void 0 : _a.some((rule) => rule.includes("@keyframes")))) {
...
var Magician = class {
  constructor(content, filename, configuration2 = {}) {
    this.configuration = {};
    this.classes = [];
    this.expressions = [];
    this.directives = [];
  ...
  prepare() {
    ...
    this.content = this.content.replace(/([!\w][\w:_/-]*?):\(([\w\s/-]*?)\)/gm, (_, groupOne, groupTwo) => groupTwo.split(/\s/g).map((cssClass) => `${groupOne}:${cssClass}`).join(" "));
    return this;
  }
  setInject() {
    let ast;
    try {
      ast = parse$3(this.content, {});
    } catch (error2) {
    }
    if (ast && !ast.css) {
      this.content += "\n<style>\n</style>\n";
  ...
  processClassAttribute() {
    ...
    for (const match of CLASS_MATCHES) {
      const cleanMatch = (_a = match.groups) == null ? void 0 : _a.classes.replace(/windi[`].+?[`]/gi, " ").replace(/(?<![-])[$](?=[{])/gi, " ").replace(/(?<=([{][\w\s]+[^{]*?))['"]/gi, " ").replace(/(?<=([{][\w\s]+[^{]*?)\s)[:]/gi, " ").replace(/([{][\w\s]+[^{]*?[?])/gi, " ").replace(/[{}]/gi, " ").replace(/\n/gi, " ").replace(/ {2,}/gi, " ").replace(/["'`]/gi, "");
      this.classes = this.classes.concat((cleanMatch || "").split(" ").filter((c2) => {
        return c2.length > 0;
      }));
    }
    return this;
  }
  processClassAttributeWithCurly() {
    var _a;
    const CLASS_MATCHES = [
  ...
  processAttributify() {
    ...
    for (const match of ATTRIBUTIFY_CLASS_MATCHES) {
      ...
      if (match[1].startsWith("class:"))
        continue;
      const cleanedMatch = match[3].replace(/windi[`].+?[`]|(?<![-])[$](?=[{])|(?<=([{][\w\s]+[^{]*?))['"]|(?<=([{][\w\s]+[^{]*?))\s[:]|([{][\w\s]+[^{]*?[?])|^[{]|(?<=["'`)])[}]/gi, " ").replace(/ {2,}/gi, " ").replace(/["'`]/gi, "").replace(/\n/gi, " ");
  ...
  getSets() {
    var _a, _b;
    let iC = new Set([]);
    if (((_a = this.configuration.experimental) == null ? void 0 : _a.icons) != void 0) {
      iC = new Set(this.classes.filter((c2) => {
        var _a2, _b2;
        return !c2.startsWith(((_b2 = (_a2 = this.configuration.experimental) == null ? void 0 : _a2.icons) == null ? void 0 : _b2.prefix) || "i-");
      }));
    } else {
    ...
    let iI = new Set([]);
    if (((_b = this.configuration.experimental) == null ? void 0 : _b.icons) != void 0) {
      iI = new Set(this.classes.filter((c2) => {
        var _a2, _b2;
        return c2.startsWith(((_b2 = (_a2 = this.configuration.experimental) == null ? void 0 : _a2.icons) == null ? void 0 : _b2.prefix) || "i-");
      }));
    }
...
async function generateCSS(key, attributes) {
  var _a;
  const t = raw.get(key);
  const defaultStyleSheet = generatorWindi.interpret(Array.from(new Set([
    ...(t == null ? void 0 : t.data.inlineClasses) || new Set(),
    ...(t == null ? void 0 : t.data.inlineDirectives) || new Set(),
    ...(t == null ? void 0 : t.data.inlineExpressions) || new Set()
  ])).join(" ")).styleSheet;
  let defaultStyles = "";
  ...
  const nObj = {};
  t == null ? void 0 : t.data.inlineAttributify.forEach((v, k) => {
    nObj[k] = Array.from(v);
  });
  ...
  let iconStyles = "";
  if (generatorUno && ((_a = configuration.experimental) == null ? void 0 : _a.icons) != void 0) {
    iconStyles = await generatorUno.generate((t == null ? void 0 : t.data.inlineIcons) || "").then((resolve2) => resolve2.css);
  }
...
function main() {
  var _a;
  if (((_a = configuration.experimental) == null ? void 0 : _a.scan) !== void 0) {
    const files = import_fast_glob.default.sync(["src/**/*.svelte"], {});
    for (const file of files) {
      const content = (0, import_fs3.readFileSync)(file).toString();
      const filename = file;
      const ast = parse$3(content, { filename });
      const hasGlobalInline = ast.css.attributes.some((el) => el.name == "windi-inline-global");
  ...
  return {
    async style({ content, attributes, filename }) {
      ...
      if (!filename)
        return { code: content };
      let preflightStyleSheet = new import_style.StyleSheet();
      if (attributes["windi:preflights:global"] || attributes["windi-preflights-global"]) {
        preflightStyleSheet = globalStyleSheet(generatorWindi.preflight());
      }
      const preflightStyles = preflightStyleSheet.build();
      let safelistStyleSheet = new import_style.StyleSheet();
      let safelistIconsStyleSheet = new import_style.StyleSheet();
      if (attributes["windi:safelist:global"] || attributes["windi-safelist-global"]) {
        safelistStyleSheet = globalStyleSheet(generatorWindi.interpret(configuration.safeList || "").styleSheet);
        if (generatorUno && ((_a2 = configuration.experimental) == null ? void 0 : _a2.icons) != void 0) {
          const { css } = await generatorUno.generate(configuration.safeList || "");
          safelistIconsStyleSheet = globalStyleSheet(new import_parser.CSSParser(css).parse());
        }
      }
      ...
      const cssStyleSheet = new import_parser.CSSParser(content, generatorWindi).parse() || new import_style.StyleSheet();
      let cssStyles = "";
      ...
      let newCode = "";
      if (preflightStyles.length > 0)
        newCode += "\n" + preflightStyles;
      if (safelistStyles.length > 0)
        newCode += "\n" + safelistStyles;
      if (defaultStyles.length > 0)
        newCode += "\n" + defaultStyles;
      if (attributifyStyles.length > 0)
        newCode += "\n" + attributifyStyles;
...
var generatorWindi;
var generatorUno;
var windiConfiguration;
function windi(userConfig = {}) {
  var _a, _b;
  configuration = __spreadValues(__spreadValues({}, defaultConfig), userConfig);
  generatorWindi = new import_lib.Processor();
  const steps = [];
  if (((_a = configuration.experimental) == null ? void 0 : _a.icons) != void 0) {
    generatorUno = createGenerator({
      presets: [
        src_default(__spreadValues({}, configuration.experimental.icons))
      ]
    }, {});
  }
  if (((_b = configuration.experimental) == null ? void 0 : _b.scan) == void 0) {
    steps.push(agent());
  }
  steps.push(main());
  return {
    async markup({ content, filename }) {
      if (!windiConfiguration) {
        ...
        if (typeof (config == null ? void 0 : config.safelist) == "string") {
          configuration.safeList = config.safelist;
        } else if (config == null ? void 0 : config.safelist) {
